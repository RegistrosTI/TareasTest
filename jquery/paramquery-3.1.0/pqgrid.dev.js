	...
	fnSB._createLayout = function() {
		...
		if (direction == "vertical") {
			...
			if (theme) {
				...
				ele[0].innerHTML = ["<div class='top-btn pq-sb-btn ui-state-default ui-corner-top'>", createButton(bts_on, bts_on ? bootstrap.up : "triangle-1-n"), "</div>", "<div class='pq-sb-slider ui-corner-all ui-state-default ", (bts_on ? bootstrap.slider : ""), "'>", "</div>", "<div class='bottom-btn pq-sb-btn ui-state-default ui-corner-bottom'>", createButton(bts_on, bts_on ? bootstrap.down : "triangle-1-s"), "</div>"].join("")
			} else {
			...
			if (theme) {
				...
				ele[0].innerHTML = ["<div class='left-btn pq-sb-btn ui-state-default ui-corner-left'>", createButton(bts_on, bts_on ? bootstrap.left : "triangle-1-w"), "</div>", "<div class='pq-sb-slider pq-sb-slider-h ui-state-default ui-corner-all ", (bts_on ? bootstrap.slider : ""), "'>", "</div>", "<div class='right-btn pq-sb-btn ui-state-default ui-corner-right'>", createButton(bts_on, bts_on ? bootstrap.right : "triangle-1-e"), "</div>"].join("")
			} else {
...
(function($) {
	...
	var cGenerateView = function(that) {
		...
		if (o.postRenderInterval != null) {
			var widgetEP = that.widgetEventPrefix.toLowerCase(),
				EN = that.eventNamespace;
			that.element.on(widgetEP + "refresh" + EN, function(evt) {
				if (self.belongs(evt)) {
					self.postRenderAll()
				}
			}).on(widgetEP + "refreshrow" + EN, function(evt, ui) {
				if (self.belongs(evt)) {
	...
	_pGenerateView.postRenderRow = function(ui) {
		var that = this.that,
			...
			finalH = that.finalH,
			postRender;
		for (var j = initH; j <= finalH; j++) {
			var column = CM[j];
			if (!column.hidden) {
				if (postRender = column.postRender) {
					ui.dataIndx = column.dataIndx;
					ui.$cell = that.getCell(ui);
					postRender.call(that.element[0], ui)
				}
			}
		}
	};
	_pGenerateView.setTimerPostRender = function(data, rowIP, finalV) {
		var that = this.that,
			self = this,
			fn = function(cb) {
				return (that.options.postRenderInterval == -1) ? cb() : window.setTimeout(cb, 0)
			};
		self.postRenderTimeoutID = fn(function() {
			...
			rowIP++;
			if (rowIP <= finalV) {
				self.setTimerPostRender(data, rowIP, finalV)
			}
		})
	};
	_pGenerateView.postRenderAll = function() {
		var that = this.that,
			...
			fn = function(cb) {
				return (intv == -1) ? cb() : window.setTimeout(cb, intv)
			};
		window.clearTimeout(self.postRenderTimeoutID);
		self.postRenderTimeoutID = fn(function() {
			self.setTimerPostRender(data, initV, finalV)
		})
	};
	...
	_pGenerateView.setPanes = function() {
		var that = this.that,
			...
			initH = that.initH,
			offset = 1,
			wd = calcWidthCols.call(that, -1, freezeCols) + offset,
			flexWidth = o.width === "flex",
	...
	_pGenerateView._generateTables = function(initV, finalV, objP) {
		...
		if (1 === 1) {
			...
			for (var col = 0; col <= finalH; col++) {
				if (col < initH && col >= freezeCols && (virtualX || other)) {
					col = initH;
					if (col > finalH) {
						throw ("initH>finalH");
						break
		...
		for (var row = 0; row <= finalRow; row++) {
			...
			if (GM && rowData.groupTitle != null) {
				...
				if (GM && rowData.groupSummary) {
					...
					this._generateRow(rowData, rowIndxPage, thisColModel, buffer, other, lastFrozenRow, nestedshow);
					if (nestedshow) {
						this._generateDetailRow(rowData, rowIndxPage, thisColModel, buffer, other, lastFrozenRow)
					}
				}
			}
		}
	...
	_pGenerateView.readCell = function(rowData, column) {
	...
	_pGenerateView.saveCell = function(rowData, column, val) {
		var dataIndx = column.dataIndx;
		if (column.nested) {
	...
	_pGenerateView._renderCell = function(objP, _dataCell) {
		var that = this.that,
			...
			column = objP.column,
			type = column.type,
			cellData = this.readCell(rowData, column);
		if (!rowData) {
			return
		}
		var dataCell;
	...
	_pGenerateView._generateRow = function(rowData, rowIndx, CM, buffer, objP, lastFrozenRow, nestedshow) {
		...
		if (numberCell.show) {
			buffer.push(["<td pq-col-indx='-1' class='pq-grid-number-cell ui-state-default'>", ((objP) ? "&nbsp;" : (rowIndx + 1)), "</td>"].join(""))
		}
		var objRender = {
			rowIndx: rowIndx + offset,
			rowIndxPage: rowIndx,
		...
		for (var col = 0; col <= finalH; col++) {
			if (virtualX && col < initH && col >= freezeCols) {
				col = initH
			}
			var column = CM[col];
			if (column.hidden) {
	...
	fn._create = function() {
		...
		this.$vscroll = $("<div class='pq-vscroll'></div>").appendTo($grid_center);
		this.prevVScroll = 0;
		...
		var prevHScroll = 0;
		var $hscroll = this.$hscroll = $("<div class='pq-hscroll'></div>").appendTo($grid_center);
		if (o.height === "flex") {
		...
		$hscroll.pqScrollBar({
			...
			scroll: function(evt, obj) {
				var virtualXHeader = o.virtualXHeader,
	...
	fn._initPager = function() {
		...
		if (PM.type) {
			if (PM.appendTo) {
				this.$pager = PM.appendTo.pqPager(obj2)
			} else {
				this.$pager = this.$footer.pqPager(obj2)
			}
		} else {}
	};
	fn._triggerPageChangeEvent = function(evt) {
		var ui = this._pageChangeEvent;
		if (ui) {
			this._trigger("pageChange", evt, ui);
			delete this._pageChangeEvent
		}
	};
	fn._delegatePageChangeEvent = function(ui) {
		this._pageChangeEvent = ui
	};
			...
			if (key === "width" && value == "flex") {
				...
				if (key == "title") {
					...
					if (key == "roundCorners") {
						...
						if (key == "virtualX") {
							this._super(key, value);
							this.$hscroll.pqScrollBar("option", "steps", value)
						} else {
							if (key == "virtualY") {
								this._super(key, value);
								this.$vscroll.pqScrollBar("option", "steps", value)
							} else {
								if (key == "freezeCols") {
									value = parseInt(value);
	...
	fn._scrollRowVirtual = function(obj) {
		...
		if (calcCurPos < scrollCurPos) {
			$vscroll.pqScrollBar("option", "cur_pos", calcCurPos).pqScrollBar("scroll")
		}
		...
		if (tr == undefined) {
			if (o.scrollModel.smooth) {
				...
				$vscroll.pqScrollBar("option", "ratio", ratio).pqScrollBar("drag")
			} else {
				$vscroll.pqScrollBar("option", "cur_pos", calcCurPos).pqScrollBar("scroll")
			}
		} else {
			...
			if ($tr_prev.hasClass("pq-row-hidden") || $tr_prev.hasClass("pq-last-frozen-row")) {
				...
				if (td_bottom > htCont - htSB - marginTop) {
					...
					var num_eles = $vscroll.pqScrollBar("option", "num_eles");
					if (num_eles < cur_pos + 1) {
						num_eles = cur_pos + 1
					}
					$vscroll.pqScrollBar("option", {
						num_eles: num_eles,
						cur_pos: cur_pos
					}).pqScrollBar("scroll")
				}
	...
	function ismergedCell(rowIndxPage, colIndx, that) {
		var rowspan = that.attr({
				rowIndxPage: rowIndxPage,
				colIndx: colIndx,
				attr: "rowspan"
			}).attr,
	...
	function findRootMergeCell(ui) {
		var that = ui.that,
			rowIndx = ui.rowIndx;
		if (ui.select && rowIndx <= that.initV || rowIndx > that.pdata.length) {
			return ui
		}
		...
		if (dataref && dataref.rowIndx != null) {
			return findRootMergeCell({
				rowIndx: dataref.rowIndx,
				colIndx: colIndx,
				that: that
			})
		} else {
	...
	fn.setSelection = function(obj) {
		...
		if (colIndx != null) {
			if (isHidden({
				...
				obj.rowIndx = obj2.rowIndx;
				obj.colIndx = obj2.colIndx;
				obj.rowIndxPage = obj2.rowIndx - this.rowIndxOffset
			} else {
				this.removeData({
					rowIndx: rowIndx,
					colIndx: colIndx,
	...
	fn.saveEditCell = function(objP) {
		...
		if (rowIndxPage != null) {
			...
			if ($.isPlainObject(newVal)) {
				...
				oldVal = GV.readCell(rowData, column)
			}
			if (newVal == "<br>") {
				newVal = ""
			}
			...
			if (1 == 1) {
				...
				var merged = ismergedCell(rowIndxPage, colIndx, this);
				if (merged && merged.rowspan) {
					var rowspan = merged.rowspan,
						colspan = merged.colspan,
						oldRow = {};
					oldRow[dataIndx] = GV.readCell(rowData, dataIndx);
					var rowList = [];
					if (rowspan) {
	...
	fn._digestData = function(ui) {
		...
		var that = this,
			...
			saveCell = this.iGenerateView.saveCell,
	...
	fn._editCell = function(objP) {
		...
		var cellData = that.iGenerateView.readCell(rowData, column),
			inp, objCall = {
				rowIndx: rowIndx,
				rowIndxPage: rowIndxPage,
				$cell: $cell,
	...
	fn.getCell = function(obj) {
		var rowIndxPage = obj.rowIndxPage,
			...
			$td;
		if ($tbl && $tbl.length) {
			$td = $tbl.children().children("tr[pq-row-indx=" + rowIndxPage + "]").children("[pq-col-indx=" + colIndx + "]")
		} else {
			$td = $()
		}
		return $td
	...
	fn.getCellHeader = function(obj) {
		...
		if ($tbl != undefined) {
			...
			var $td = $tbl.children().children("tr.pq-grid-title-row:last").children("[pq-col-indx=" + colIndx + "]")
		}
	...
	_pKeyNav._bodyKeyPressDown = function(evt) {
		...
		if (keyCode == KC.LEFT || keyCode == KC.RIGHT || keyCode == KC.UP || keyCode == KC.DOWN) {
			var merged = ismergedCell(rowIndxPage, colIndx, that),
				pqN;
			if (merged) {
				pqN = that.data({
					rowIndx: rowIndx,
...
(function($) {
	...
	_p.createHeader = function() {
		...
		var tblClass = (bootstrap.on ? bootstrap.thead : "") + " pq-grid-header-table ";
		if (hwrap) {
			tblClass += "pq-wrap "
		} else {
			tblClass += "pq-no-wrap "
		...
		$header_o.empty();
		if (pqpanes.vH) {
			$header_o[0].innerHTML = (["<span class='pq-grid-header pq-grid-header-left ui-state-default'>", "<div class='pq-grid-header-inner'>", strTbl, "</div>", "</span>", "<span class='pq-grid-header ui-state-default'>", "<div class='pq-grid-header-inner'>", strTbl, "</div>", "</span>"].join(""))
		} else {
			$header_o[0].innerHTML = (["<span class='pq-grid-header ui-state-default'>", "<div class='pq-grid-header-inner'>", strTbl, "</div>", "</span>"].join(""))
		}
	...
	_p.refreshHeaderSortIcons = function() {
		...
		$tds.removeClass("pq-col-sort-asc pq-col-sort-desc ui-state-active active");
		...
		for (var i = 0; i < sorterLen; i++) {
			var sorter = sorters[i],
				...
				dir = sorter.dir,
				addClass = "active ui-state-active pq-col-sort-" + (dir == "up" ? "asc" : "desc"),
				cls2 = "ui-icon ui-icon-triangle-1-" + (dir == "up" ? "n" : "s") + " glyphicon glyphicon-arrow-" + dir;
	...
	var cResizeColumns = function(that) {
		...
		if (flex.on && flex.one) {
			var widgetEventPrefix = that.widgetEventPrefix.toLowerCase(),
				EN = that.eventNamespace;
			that.element.one(widgetEventPrefix + "dataready" + EN, function() {
				$(this).one(widgetEventPrefix + "refresh" + EN, function(evt) {
					if (self.belongs(evt)) {
						self.flex()
					}
				})
	...
	_pResizeColumns.flex = function(objP) {
		...
		for (var i = 0; i <= finalH; i++) {
			if (virtualX && i < initH && i >= freezeCols) {
				i = initH
			}
			var column = CM[i];
			if (column.hidden) {
...
(function($) {
	...
	_pRefresh.setContAndGridHeightForFlex = function() {
		var that = this.that;
		if (this.vscroll) {
			that.$hscroll.css("position", "")
		} else {
			that.$hscroll.css("position", "relative");
			var $cont = that.$cont,
	...
	_pRefresh.calcInitFinalH = function() {
		...
		if (!virtualX) {
			...
			var freezeCols = parseInt(o.freezeCols),
				flexWidth = o.width === "flex",
				initH = freezeCols,
				indx = 0;
			for (var i = freezeCols; i < CMLength; i++) {
...
(function($) {
	...
	fn.loadState = function(ui) {
		...
		var CM = this.colModel,
			...
			getColumn = function(dataIndx) {
				for (var i = 0, len = CM.length; i < len; i++) {
					var column = CM[i];
					if (dataIndx == column.dataIndx) {
						return column
					}
				}
			};
		for (var i = 0, len = CMstate.length; i < len; i++) {
			var columnS = CMstate[i],
				dataIndx = columnS.dataIndx,
				column = getColumn(dataIndx);
			$.extend(true, column, columnS);
			$.extend(true, columnS, column)
		}
		this.option({
			colModel: CMstate,
			height: state.height,
	...
	fn._create = function() {
		...
		this._queueATriggers = {};
		this._queueBTriggers = {};
		this.iHistory = new pq.cHistory(this);
		this.iGroupView = new cGroupView(this);
		if (pq.cMerge) {
			this.iMerge = new pq.cMerge(this)
		}
		this.iFilterData = new pq.cFilterData(this);
		this.iHeaderSearch = new cHeaderSearch(this);
		this.iUCData = new pq.cUCData(this);
		this.iMouseSelection = new pq.cMouseSelection(this);
		this._super();
		this.iGroupView.bindEvents();
		this.iDragColumns = new pq.cDragColumns(this);
		this._createToolbar();
...
(function($) {
	...
	fnGrid.updateRow = function(objP) {
		...
		for (var dataIndx in newRow) {
			...
			if (column) {
				oldRow[dataIndx] = this.iGenerateView.readCell(rowData, column)
			}
		}
		var ret = this._digestData({
...
(function($) {
	...
	_pCheckBC.onHeaderClick = function(evt) {
		...
		if (finalChk) {
			...
			that.$tbl.children().children().children("[pq-col-indx=" + colIndx + "]").find("input").prop("checked", true);
			if (that._trigger("check", evt, ui) !== false && cb.select) {
				self.selectAllRows()
			}
		} else {
			...
			that.$tbl.children().children().children("[pq-col-indx=" + colIndx + "]").find("input").prop("checked", false);
			if (that._trigger("unCheck", evt, ui) !== false && cb.select) {
...
(function($) {
	...
	_pMouseSelection.syncViewWithScrollBarVert = function(ratio) {
		...
		if (!scrollHt || !contHt) {
			if (o.debug) {
				throw "_syncViewWithScrollBarVert !scrollHt || !contHt"
			}
		}
		if (scrollTop < 0) {
			scrollTop = 0
...
(function($) {
	...
	_pExcel.copy = function() {
		var $text = $("#pq-grid-excel"),
			...
			CM = that.colModel,
			CMLength = CM.length,
			readCell = that.iGenerateView.readCell,
			selArr = [],
			buffer = [];
		if (!CPM.on) {
			return
		...
		for (var i = 0, len = selArr.length; i < len; i++) {
			...
			for (var j = 0; j < CMLength; j++) {
				...
				rowBuffer.push(readCell(rowData, column))
			}
