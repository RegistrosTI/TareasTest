...
(function($) {
	...
	fn.refreshDataFromDataModel = function(obj) {
		...
		var thisOptions = this.options,
			...
			DMdata = DM.data,
			begIndx, endIndx, totalPages, paging = PM.type,
			qTriggers = this._queueATriggers;
		...
		if (paging == "local") {
			...
			PM.totalPages = totalPages = Math.ceil(DMdata.length / PM.rPP);
...
(function($) {
	...
	_pCompute.summaryData = function() {
		var self = this,
			formulas = self.formulas,
			flen = formulas.length,
			sdata, i;
		if (flen) {
			sdata = self.that.options.summaryData || [];
			i = sdata.length;
			while (i--) {
				self.callRow(sdata[i], formulas, flen)
			}
		}
	};
...
(function($) {
	...
	_p.getXlsContent = function(CM, CMLen, hc, hcLen, data, dataLen, remotePage, offset, iMerge, render, iGV, header) {
		var self = this,
			frozen = '<sheetViews><sheetView tabSelected="1" workbookViewId="0"><pane ySplit="<%=a%>" topLeftCell="A<%=b%>" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews>',
...
(function($) {
	...
	_pSort.readSorter = function() {
		var that = this.that,
			o = that.options,
			GM = o.groupModel,
			GMdataIndx, dataIndx, columns = that.columns,
			i = 0,
			len, GMDir, sorters = [];
		if (GM.on) {
			...
			GMDir = GM.dir;
			for (; i < len; i++) {
				dataIndx = GMdataIndx[i];
				sorters.push({
					dataIndx: dataIndx,
					dir: (GMDir[i] ? GMDir[i] : "up")
				})
			}
		}
		var SM = o.sortModel,
	...
	_pSort.refreshSorter = function(ui) {
		...
		var that = this.that,
			...
			GM = o.groupModel,
			GMdataIndx = GM.dataIndx,
			GMLength = GM.on ? GMdataIndx.length : 0,
			foundInGMIndx = -1,
	...
	_pSort._sortLocalData = function(sorters, data) {
		...
		if (data.length == 0 || !sorters || !sorters.length) {
			return data
		}
		var self = this,
			that = self.that,
...
(function($) {
	...
	_pGroup.init = function() {
		...
		if (!self._init) {
			...
			that.on("cellClick", self.onCellClick(self)).on("cellKeyDown", self.onCellKeyDown(self, GM)).on(true, "cellMouseDown", self.onCellMouseDown()).on("change", self.onChange(self, GM)).on("dataReady", self.onDataReady(self, that)).on("beforeSortDone", self.onBeforeSort(self, that)).on("columnDragDone", self.onColumnDrag(self)).on("columnOrder", self.onColumnOrder(self));
			self._init = true
		}
	};
	...
	_pGroup.pq_gc_ = function() {
		return "pq_gc_"
	};
	_pGroup.onBeforeSort = function(self, that) {
		return function(evt) {
			...
			var columns = that.columns,
				columnST = [],
				data = o.dataModel.data || [],
				tmp_dataIndx, tmp_dataIndx_arr = [],
				rd, i, GMDataIndx = GM.dataIndx,
				j = GMDataIndx.length;
			while (j--) {
				var dataIndx = GMDataIndx[j],
					column = columns[dataIndx],
					chg = column.groupChange;
				if (chg) {
					chg = pq.getFn(chg);
					columnST.push(column);
					tmp_dataIndx = self.pq_gc_() + dataIndx;
					tmp_dataIndx_arr.push(tmp_dataIndx);
					column.sortType = self.sortType(tmp_dataIndx);
					i = data.length;
					while (i--) {
						rd = data[i];
						rd[tmp_dataIndx] = chg(rd[dataIndx])
					}
				}
			}
			that.one("sort", function() {
				j = columnST.length;
				if (j) {
					while (j--) {
						columnST[j].sortType = undefined
					}
				}
			})
		}
	};
	_pGroup.sortType = function(tmp_di) {
		return function(rd1, rd2, dataIndx) {
			if (rd1[tmp_di] != rd2[tmp_di]) {
				return (rd1[dataIndx] - rd2[dataIndx])
			}
			return 0
		}
	};
	...
	_pGroup.refreshColumns = function() {
		...
		if (GM_on) {
			for (var i = groupIndxLen - 1; i >= 0; i--) {
	...
	_pGroup.showHideRows = function(initIndx, level, hide, merge) {
		var arr = [],
			that = this.that,
			collapsed, rd, data = that.pdata;
			...
			if (rd.pq_gsummary) {
				...
				if (rd.pq_gtitle) {
					...
					if (rd.pq_level <= level) {
						...
						rd.pq_hidden = (merge && !collapsed) ? true : hide
					}
				} else {
					rd.pq_hidden = hide
				}
	...
	_pGroup.trigger = function(ui) {
		var evt = ui.evt,
			...
			_close = ui.close,
			that = this.that,
			close, level, di, val, i, GM = that.options.groupModel,
			groupIndx = GM.dataIndx,
			collapsed = GM.collapsed,
	...
	_pGroup._initmerge = function() {
		...
		for (var lev = 0; lev < tree.length; lev++) {
			_tree = tree[lev];
			ci = that.getColIndx({
				dataIndx: o.groupModel.dataIndx[lev]
			});
	...
	_pGroup._initcollapsed = function() {
		var that = this.that,
			merge = that.options.groupModel.merge,
			o_pdata = this.pdata,
			pdata = that.pdata,
	...
	_pGroup._groupChange = function(ignoreCase) {
		var trim = $.trim,
			pq_gc_ = this.pq_gc_();
		return function(rd, dataIndx, column) {
			var val;
			if (column.groupChange) {
				val = rd[pq_gc_ + dataIndx];
				return (val != null) ? val : ""
			} else {
				val = trim(rd[dataIndx]);
	...
	_pGroup._groupData = function() {
		var self = this,
			...
			GM = o.groupModel,
			merge = GM.merge,
			grandSummary = GM.grandSummary,
			inArray = $.inArray,
			CM = that.colModel,
			CMLength = CM.length,
			_groupChange = self._groupChange(GM.ignoreCase),
			GMdataIndx = GM.dataIndx,
			...
			columns = that.columns,
			columnG = [],
			colDefLen, groupSummaryShow = GM.showSummary || [],
			summaryRIP = [];
		if (data && data_length > 0) {
			for (var l = GMLength - 1; l >= 0; l--) {
				groupSummaryShow[l] = groupSummaryShow[l] != null ? groupSummaryShow[l] : false;
				GMcollapsed[l] = GMcollapsed[l] != null ? GMcollapsed[l] : false;
				summaryRIP[l] = (summaryRIP[l + 1] ? summaryRIP[l + 1] : 0) + groupSummaryShow[l];
				columnG[l] = columns[GMdataIndx[l]]
			}
			for (var v = 0; v < CMLength; v++) {
				var column = CM[v],
					dataIndx = column.dataIndx,
					summary = column.summary;
				if (summary && summary.type && inArray(dataIndx, GMdataIndx) == -1) {
					colDef.push({
						column: column
					});
					cells[dataIndx] = Array(data_length - 1)
				}
			}
			colDefLen = colDef.length;
			var dataGM = [],
				rowIndxGM = [],
				tree = [],
				groupVal = [],
			...
			for (var i = 0; i <= data_length; i++) {
				var rowData = data[i],
					changeGroup = false,
					changeGroupLevel = null;
				for (var l = 0; l < GMLength; l++) {
					groupVal[l] = (i < data_length) ? _groupChange(rowData, GMdataIndx[l], columnG[l]) : "";
					if (prevGroupVal[l] != groupVal[l] || i == data_length) {
						changeGroup = true;
						if (changeGroupLevel == null) {
							changeGroupLevel = l
						}
					}
					if (changeGroup) {
						var _tree = tree[l],
							lastNode = _tree[_tree.length - 1];
						if (lastNode) {
							lastNode.rip2 = dataGM.length + summaryRIP[l]
						}
						_tree.push({
							rowIndx: i,
							title: groupVal[l]
				...
				if (changeGroup) {
					if (i > 0) {
						for (var l = GMLength - 1; l >= changeGroupLevel; l--) {
							var groupRowData = {},
								_tree = tree[l],
								indxGM = (_tree.length - 2 ? _tree[_tree.length - 2].rip : l),
								indx1 = (_tree.length - 2 ? _tree[_tree.length - 2].rowIndx : 0),
								indx2 = _tree[_tree.length - 1].rowIndx;
							for (var f = 0; f < colDefLen; f++) {
								var column = colDef[f].column,
									summary = column.summary,
									summaryType = summary.type,
									dataIndx = column.dataIndx,
									summaryCellData = "",
									_cells = cells[dataIndx].slice(indx1, indx2);
								summaryCellData = _aggr[summaryType](_cells, column);
								groupRowData[dataIndx] = summaryCellData
							}
							if (groupSummaryShow[l]) {
								var summaryRow = {
									pq_gsummary: true,
									pq_level: l,
									pq_rowcls: "pq-summary-row"
								};
								for (var key in groupRowData) {
									summaryRow[key] = groupRowData[key]
								}
								dataGM.push(summaryRow)
							}
							var groupTitleRow = dataGM[indxGM];
							for (var key in groupRowData) {
								groupTitleRow[key] = groupRowData[key]
							}
							if (merge) {
								groupTitleRow.pq_rowcls = "pq-summary-row"
							}
							groupTitleRow.pq_items = indx2 - indx1
						}
					}
					if (i == data_length) {
						break
					...
					for (var l = changeGroupLevel; l < GMLength; l++) {
						var rowTitle = {
							pq_gtitle: true,
							pq_level: l,
							pq_close: GMcollapsed[l]
						};
						rowTitle[GMdataIndx[l]] = groupVal[l];
						var dataGM_len = dataGM.push(rowTitle);
						rowIndxGM[l] = dataGM_len - 1;
						var _tree = tree[l];
						_tree[_tree.length - 1].rip = dataGM_len - 1
					}
				}
				if (i == data_length) {
					break
				...
				if (rowData.pq_hidden) {
					rowData.pq_hidden = undefined
				}
				dataGM.push(rowData);
				for (var k = 0; k < colDefLen; k++) {
					var column = colDef[k].column,
						dataIndx = column.dataIndx;
					cells[dataIndx][i] = rowData[dataIndx]
				}
			}
			self.tree = tree;
			that.pdata = dataGM
		...
		if (grandSummary) {
			...
			for (var f = 0; f < colDefLen; f++) {
				var column = colDef[f].column,
					...
					dataIndx = column.dataIndx,
					summaryCellData = "",
					_cells = cells[dataIndx];
				summaryCellData = _aggr[summaryType](_cells, column);
				groupRowData[dataIndx] = summaryCellData
			}
			self.summaryData = o.summaryData = [groupRowData]
		} else {
			self.summaryData.length = 0
		}
	};
	...
	fn.groupOption = function(ui, refresh) {
		...
		if (on && (diLength || GMdataIndx.length)) {
			sort = true
		}
