 ...
 * AUTOR: Alberto Ruiz
 * FECHA: 22/09/2017
 * DESC: ENVÍA UN CORREO A EVALUADORES CON TODAS LAS TAREAS QUE NO TIENEN APORTE JEFE
 * ***********************************************************
 */
...
// ******************************************************************************************************** VARIABLES
$asunto = "EFICIENCIA. Reporte semanal de tareas pendientes de evaluar.";
$mensaje = "";
$ASIGNADOA = "";
$TO = "";
...
// BUSCAMOS LOS USUARIOS QUE LES TENGA QUE LLEGAR EL MENSAJE
$query = "
	SELECT 
	...
	WHERE USU.Baja = 0 AND USU.InfPendEvaluar = 1
";
...
for(; DBNext ( $query ) ;) {
	...
	<body>
		...
		<p>A continuación se muestran todas las evaluaciones pendientes hasta la fecha de sus tareas en el portal:</p>
		<div class='datagrid'>
		<table>
			<thead>
				<TR>
	...
	$query2 = "
		...
		WHERE EVA.Usuario = '$USUARIO' 
			AND OFI.Evaluacion = EVA.Periodo
			AND EVA.Med_Calidad = -1
			AND EVA.Automatica = 0
			AND TAR.Control = 0 -- 27/07/18 Alberto tarea 26414 
		ORDER BY EVA.Tarea DESC
	";
	//die ($query2);
