...
include "../../soporte/cfgGestionPDTTest.php";
...
{
  GraphMailTareas::Oninit();
}

class GraphMailTareas
{

    private static string $url = "https://graph.sp-berner.com";
    private static string $id_user = 'a0cec894-8f04-442c-ba5d-fc61a2683813';
    private static string $id_folder_acepted = "AAMkADllYTk0MGFhLWZmOGYtNDc0ZS04NDVhLWUxMGMxNmE0NTA1OAAuAAAAAABTaoF15fvNQrNBsYu9CRk4AQBYr38iOz4zTKVrD6-NHTzKAABp6WaJAAA=";
    private static string $id_folder = 'AAMkADllYTk0MGFhLWZmOGYtNDc0ZS04NDVhLWUxMGMxNmE0NTA1OAAuAAAAAABTaoF15fvNQrNBsYu9CRk4AQBYr38iOz4zTKVrD6-NHTzKAAAA0OkPAAA=';
 ...
 public static function Oninit(): void{
    $callUrl = GraphMailTareas::$url . '?type=mailFolder&id_user=' . GraphMailTareas::$id_user . '&id_folder=' . GraphMailTareas::$id_folder . '&';
    $response = GraphMailTareas::CallGraph($callUrl, true);
    ...
    if ($emails) {
        ...
        foreach ($emails as $email) {
            ...
            $UsuarioPermitido = GraphMailTareas::comprobacionDominiosPermitidos($email->sender->emailAddress->address);
    
            $comprobarDestinatarios = false;
            $comprobarCorreo = false;
    
            $comprobarDestinatarios = GraphMailTareas::comprobacionDestinatarios($email);
            if ($UsuarioPermitido == true) {
    
                //$comprobarCopiasOcultas = comprobacionCopiasOcultas($header); //SE PERMITE COPIAS/COPIAS CC
                $comprobarCorreo = GraphMailTareas::comprobarcionCorreo($email);
    
    
                if (($comprobarDestinatarios == true) && ($comprobarCorreo == true)) {
    
                    GraphMailTareas::montarSQLCorreo($email);
                    GraphMailTareas::moverCorreoCarpetaAceptado($email);
    
                } else {
                    GraphMailTareas::borrarCorreo($email);
                }
            }
        }
    } else {
 ...
 public static function CallGraph($URL, $bool){  
...
$response = curl_exec($curl);
 ...
 if($bool === true){
    $response = GraphMailTareas::cleanResponse($response);
 }
  return $response;
}

 public static function cleanResponse($response): object{
...
public static function comprobacionCopiasOcultas($DestinatariosOcultos)
{
...
public static function comprobacionDestinatarios($email)
{
...
public static function comprobarUsuarioDominio($correo)
{
...
public static function comprobarcionCorreo($email)
{
...
public static function borrarCorreo($message)
{
...
public static function montarSQLCorreo($email)
{
    $message = $email->body->content;
    $g = "$message";
    $cadena_buscada = 'high';
    ...
    $raw_email_all = GraphMailTareas::getrawmessage($email);
    ...
    GraphMailTareas::enviarmail($from, "Se ha creado la solicitud para su valoración.", "Su solicitud de tarea con asunto : " . imap_utf8($subject) . "<br/><br/> Ha sido creada en el sistema para su valoración <br/><br/> <br/> <br/> <br/> <u><b> Este correo se envía de forma automática, por favor no responda a este mensaje. <br/> La creación de la solicitud es automática, pero la creación de tareas está bajo valoración.</b></u>", $mensajeid);
...
public static function moverCorreoCarpetaAceptado($email)
{
    $callUrl = GraphMailTareas::$url . '?type=move&id_user=' . GraphMailTareas::$id_user . '&id_folder=' . GraphMailTareas::$id_folder . '&id_message=' . $email->id . '&id_folder_move=' .GraphMailTareas::$id_folder_acepted.'&';
    $response = GraphMailTareas::CallGraph($callUrl, false);

    return $response;
}

public static function getrawmessage($email)
{
    $callUrl = GraphMailTareas::$url . '?type=mailraw&id_user=' . GraphMailTareas::$id_user . '&id_folder=' . GraphMailTareas::$id_folder . '&id_message=' . $email->id . '&raw_message='.GraphMailTareas::$value.'&';
    $response = GraphMailTareas::CallGraph($callUrl, false);

    return $response;
}

public static function comprobacionDominiosPermitidos($address)
{
...
public static function enviarmail($to, $asunto, $mensaje, $correoid)
{
